...
module.exports = async function(req, profile, done) {
  // profile = the data returned by the vk api
  const userToConnect = req ? req.user : undefined;
  
  const providerNameId = makeProviderId(profile);   // "vkontakte:123456"
  ...
  console.log('user to be used for merging / saving \n', user);
...
function mergeProfile(user, profile) {
  ...
  if (!user.email && ((profile.emails && profile.emails.length) || profile.email)) {
    user.email = profile.emails[0].value || profile.email; // may be many emails
  }
